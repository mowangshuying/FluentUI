...
void FluAllSamplesPage::makeBasicInputCards()
{
    ...
    auto comboBoxCard = new FluHCard(QPixmap("../res/ControlImages/ComboBox.png"), tr("ComboBox"), tr("A drop-down list of items a user can select from."));
...
void FluAllSamplesPage::makeCollectionsCards()
{
    ...
    auto listViewCard = new FluHCard(QPixmap("../res/ControlImages/ListView.png"), tr("ListView"), tr("A control that presents a collection of items in a vertical list."));
    listViewCard->setKey("ListViewPage");
    m_cards.push_back(listViewCard);

    auto treeViewCard = new FluHCard(QPixmap("../res/ControlImages/TreeView.png"), tr("TreeView"), tr("The TreeView control is a hierarchical list pattern with expanding and collapsing nodes that contain nested items."));
...
{
    auto calendarDatePickerCard = new FluHCard(QPixmap("../res/ControlImages/CalendarDatePicker.png"), tr("CalendarDatePicker"), tr("A control that lets users pick a date value using a calendar."));
    calendarDatePickerCard->setKey("CalendarDatePickerPage");
    m_cards.push_back(calendarDatePickerCard);

    auto calendarViewCard = new FluHCard(QPixmap("../res/ControlImages/CalendarView.png"), tr("CalendarView"), tr("A control that presents aa calendar for a user to choose a date from."));
    calendarViewCard->setKey("CalendarViewPage");
    m_cards.push_back(calendarViewCard);

    auto datePickerCard = new FluHCard(QPixmap("../res/ControlImages/DatePicker.png"), tr("DatePicker"), tr("A control that lets a user pick a date value."));
    datePickerCard->setKey("DatePickerPage");
    m_cards.push_back(datePickerCard);

    auto timePickerCard = new FluHCard(QPixmap("../res/ControlImages/TimePicker.png"), tr("TimePicker"), tr("A configurable control that lets a user pick a time value."));
...
{
    auto contentDialogCard = new FluHCard(QPixmap("../res/ControlImages/ContentDialog.png"), tr("ContentDialog"), tr("A dialog box that can be customized to contain any XAML content."));
    contentDialogCard->setKey("ContentDialogPage");
    m_cards.push_back(contentDialogCard);

    auto flyoutCard = new FluHCard(QPixmap("../res/ControlImages/Flyout.png"), tr("Flyout"), tr("Shows contextual information and enables user interaction."));
    flyoutCard->setKey("FlyoutPage");
    m_cards.push_back(flyoutCard);

    auto teachingTipCard = new FluHCard(QPixmap("../res/ControlImages/TeachingTip.png"), tr("TeachingTip"), tr("A content-rich flyout for guiding users and enabling teaching moments."));
...
{
    auto borderCard = new FluHCard(QPixmap("../res/ControlImages/Border.png"), tr("Border"), tr("A container control that draws a boundary line, background, or both, around another object."));
    borderCard->setKey("BorderPage");
    m_cards.push_back(borderCard);

    auto canvasCard = new FluHCard(QPixmap("../res/ControlImages/Canvas.png"), tr("Canvas"), tr("A layout panel that supports absolute positioning of child elements relative to the top left corner of the canvas."));
    canvasCard->setKey("CanvasPage");
    m_cards.push_back(canvasCard);

    auto expanderCard = new FluHCard(QPixmap("../res/ControlImages/Expander.png"), tr("Expander"), tr("A container with a header that can be expanded to show a body with more content."));
    expanderCard->setKey("ExpanderPage");
    m_cards.push_back(expanderCard);

    auto gridCard = new FluHCard(QPixmap("../res/ControlImages/Grid.png"), tr("Grid"), tr("A flexible, primitive control for data-driven layouts."));
    gridCard->setKey("GridPage");
    m_cards.push_back(gridCard);

    auto itemsRepeaterCard = new FluHCard(QPixmap("../res/ControlImages/ItemsRepeater.png"), tr("ItemsRepeater"), tr("A flexible, primitive control for data-driven layouts."));
    itemsRepeaterCard->setKey("ItemRepeater");
    m_cards.push_back(itemsRepeaterCard);

    auto radioButtonsCard = new FluHCard(QPixmap("../res/ControlImages/RadioButtons.png"), tr("RadioButtons"), tr("A control that displays a group of mutually exclusive options with keyboarding and accessibility support."));
    radioButtonsCard->setKey("RadioButtonsPage");
    m_cards.push_back(radioButtonsCard);

    auto relativePanelCard = new FluHCard(QPixmap("../res/ControlImages/RelativePanel.png"), tr("RelativePanel"), tr("A panel that users relationships between elements to define layout."));
    relativePanelCard->setKey("RelativePanelPage");
    m_cards.push_back(relativePanelCard);

    auto splitViewCard = new FluHCard(QPixmap("../res/ControlImages/SplitView.png"), tr("SplitView"), tr("A container that has 2 content areas, with multiple display options for the pane."));
    splitViewCard->setKey("SplitViewPage");
    m_cards.push_back(splitViewCard);

    auto stackPanelCard = new FluHCard(QPixmap("../res/ControlImages/StackPanel.png"), tr("StackPanel"), tr("A layout panel that arranges child elements into a single line that can be oriented horizontally or vertically."));
    stackPanelCard->setKey("StackPanelPage");
    m_cards.push_back(stackPanelCard);

    auto variableSizedWrapGridCard =
        new FluHCard(QPixmap("../res/ControlImages/VariableSizedWrapGrid.png"), tr("VariableSizedWrapGrid"), tr("A layout panel that supports arranging child elements in rows and columns. Each child element can span multiple rows and columns."));
    variableSizedWrapGridCard->setKey("VariableSizedWrapGridPage");
    m_cards.push_back(variableSizedWrapGridCard);

    auto viewboxCard = new FluHCard(QPixmap("../res/ControlImages/Viewbox.png"), tr("Viewbox"), tr("A container control that scales its content to a specified size."));
...
{
    auto appBarButtonCard = new FluHCard(QPixmap("../res/ControlImages/AppBarButton.png"), tr("AppBarButton"), tr("A button that's styled for use in a CommandBar."));
    appBarButtonCard->setKey("AppBarButtonPage");
    m_cards.push_back(appBarButtonCard);

    auto appBarSeparatorCard = new FluHCard(QPixmap("../res/ControlImages/AppBarSeparator.png"), tr("AppBarSeparator"), tr("A vertical line that's used to visually separate groups of commands in an app bar."));
    ...
    auto menuFlyoutCard = new FluHCard(QPixmap("../res/ControlImages/MenuFlyout.png"), tr("MenuFlyout"), tr("Shows a contextual list of simple command or options."));
    ...
    auto swipeControlCard = new FluHCard(QPixmap("../res/ControlImages/SwipeControl.png"), tr("SwipeControl"), tr("Touch gesture for quick menu actions on items."));
...
void FluAllSamplesPage::makeNavigationCards()
{
    ...
    auto navigationViewCard = new FluHCard(QPixmap("../res/ControlImages/NavigationView.png"), tr("NavigationView"), tr("Common vertical layout for top-level areas of your app via a collapsible navigation menu."));
    navigationViewCard->setKey("NavigationViewPage");
    m_cards.push_back(navigationViewCard);

    auto pivotCard = new FluHCard(QPixmap("../res/ControlImages/Pivot.png"), tr("Pivot"), tr("Presents information from different sources in a tabbed view."));
    pivotCard->setKey("PivotPage");
    m_cards.push_back(pivotCard);

    auto selectorBarCard = new FluHCard(QPixmap("../res/ControlImages/Pivot.png"), tr("SelectorBar"), tr("Presents information from a small set of different sources. The user can pick one of them."));
    selectorBarCard->setKey("SelectorBarPage");
    m_cards.push_back(selectorBarCard);

    auto tabViewCard = new FluHCard(QPixmap("../res/ControlImages/TabView.png"), tr("TabView"), tr("A control that displays a collection of tabs that can be used to display several documents."));
...
void FluAllSamplesPage::makeScrollingCards()
{
    ...
    auto pipsPagerCard = new FluHCard(QPixmap("../res/ControlImages/PipsPager.png"), tr("PipsPager"), tr("A control to let the user navigate through a paginated collection when the page numbers do not need to be visually known."));
    pipsPagerCard->setKey("PipsPagerPage");
    m_cards.push_back(pipsPagerCard);

    auto scrollViewCard = new FluHCard(QPixmap("../res/ControlImages/ScrollView.png"), tr("ScrollView"), tr("A container control that lets the user pan and zoom its content."));
    scrollViewCard->setKey("ScrollViewPage");
    m_cards.push_back(scrollViewCard);

    auto scrollViewerCard = new FluHCard(QPixmap("../res/ControlImages/ScrollViewer.png"), tr("ScrollViewer"), tr("A container control that lets the user pan and zoom its content."));
    scrollViewerCard->setKey("ScrollViewerPage");
    m_cards.push_back(scrollViewerCard);

    auto semanticZoomCard = new FluHCard(QPixmap("../res/ControlImages/SemanticZoom.png"), tr("SemanticZoom"), tr("Lets the user zoom between two different views of a collection, making it easier to navigate through large collections of items.."));
...
{
    auto infoBadgeCard = new FluHCard(QPixmap("../res/ControlImages/InfoBadge.png"), tr("InfoBadge"), tr("An Non-instrusive Ui to display notifications or bring focus to an area."));
    infoBadgeCard->setKey("InfoBadgePage");
    m_cards.push_back(infoBadgeCard);

    auto infoBarCard = new FluHCard(QPixmap("../res/ControlImages/InfoBar.png"), tr("InfoBar"), tr("An inline message to display app-wide status change information."));
    infoBarCard->setKey("InfoBarPage");
    m_cards.push_back(infoBarCard);

    auto progressBarCard = new FluHCard(QPixmap("../res/ControlImages/ProgressBar.png"), tr("ProgressBar"), tr("Shows the apps progress on a task, or that the app is performing ongoing work that doesn't block user interaction."));
    progressBarCard->setKey("ProgressBarPage");
    m_cards.push_back(progressBarCard);

    auto progressRingCard = new FluHCard(QPixmap("../res/ControlImages/ProgressRing.png"), tr("ProgressRing"), tr("Shows the apps progress on a task, or that the app is performing ongoing work that does block user interaction."));
...
void FluAllSamplesPage::makeTextCards()
{
    ...
    auto richEditBoxCard = new FluHCard(QPixmap("../res/ControlImages/RichEditBox.png"), tr("RichEditBox"), tr("A rich text editing control that supports formatted text, hyperlinks and other rich content."));
    richEditBoxCard->setKey("RichEditBoxPage");
    m_cards.push_back(richEditBoxCard);

    auto richTextBlockCard = new FluHCard(QPixmap("../res/ControlImages/RichTextBlock.png"), tr("RichTextBlock"), tr("A control that displays formatted text hyperlinks, inline images, and other rich content."));
    ...
    auto textBoxCard = new FluHCard(QPixmap("../res/ControlImages/TextBox.png"), tr("TextBox"), tr("A single-line or multi-line plain text field."));
