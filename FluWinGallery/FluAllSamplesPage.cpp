...
void FluAllSamplesPage::makeBasicInputCards()
{
    ...
    auto checkboxCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/Checkbox.png")), tr("CheckBox"), tr("A control that a user can select or clear."));
    checkboxCard->setKey("CheckBoxPage");
    m_cards.push_back(checkboxCard);

    auto colorPickerCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ColorPicker.png")), tr("ColorPicker"), tr("A control that displays a selectable color spectrum."));
    colorPickerCard->setKey("ColorPickerPage");
    m_cards.push_back(colorPickerCard);

    auto comboBoxCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ComboBox.png")), tr("ComboBox"), tr("A drop-down list of items a user can select from."));
    comboBoxCard->setKey("ComboBoxPage");
    m_cards.push_back(checkboxCard);

    auto dropDownButtonCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/DropDownButton.png")), tr("DropDownButton"), tr("A button that displays a flyout of choices when clicked."));
    dropDownButtonCard->setKey("DropDownButtonPage");
    m_cards.push_back(dropDownButtonCard);

    auto hyperlinkButtonCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/HyperlinkButton.png")), tr("HyperlinkButton"), tr("A button that appears as hyperlink text and can navigate to URI or handle a Click event."));
    hyperlinkButtonCard->setKey("HyperlinkButtonPage");
    m_cards.push_back(hyperlinkButtonCard);

    auto radioButtonCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/RadioButton.png")), tr("RadioButton"), tr("A control that allows a user to select a single option from a group of options."));
    radioButtonCard->setKey("RadioButtonPage");
    m_cards.push_back(radioButtonCard);

    auto ratingControlCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/RatingControl.png")), tr("RatingControl"), tr("Rate something 1 to 5 stars."));
    ratingControlCard->setKey("RatingControlPage");
    m_cards.push_back(ratingControlCard);

    auto repeatButtonCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/RepeatButton.png")), tr("RepeatButton"), tr("A button that raisers its Click event repeatedly from the time it's pressed until it's released."));
    repeatButtonCard->setKey("RepeatButtonPage");
    m_cards.push_back(repeatButtonCard);

    auto sliderCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/Slider.png")), tr("Slider"), tr("A control that lets the user select from a range of values by moving a Thumb control along a track."));
    sliderCard->setKey("SliderPage");
    m_cards.push_back(sliderCard);

    auto splitButtonCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/SplitButton.png")), tr("SplitButton"), tr("A two-part button that display a flyout when its secondary part is clicked."));
    splitButtonCard->setKey("SplitButtonPage");

    auto toggleButtonCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ToggleButton.png")), tr("ToggleButton"), tr("A button that can be switched between two states like a CheckBox."));
    toggleButtonCard->setKey("ToggleButtonPage");
    m_cards.push_back(toggleButtonCard);

    auto toggleSplitButtonCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ToggleSplitButton.png")), tr("ToggleSplitButton"), tr("A version of the SplitButton where the activation target toggles on/off"));
    toggleSplitButtonCard->setKey("ToggleSplitButtonPage");
    m_cards.push_back(toggleSplitButtonCard);

    auto toggleSwitchCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ToggleSwitch.png")), tr("ToggleSwitch"), tr("A switch that can be toggled between 2 states."));
...
void FluAllSamplesPage::makeCollectionsCards()
{
    ...
    auto listViewCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ListView.png")), tr("ListView"), tr("A control that presents a collection of items in a vertical list."));
    listViewCard->setKey("ListViewPage");
    m_cards.push_back(listViewCard);

    auto treeViewCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/TreeView.png")), tr("TreeView"), tr("The TreeView control is a hierarchical list pattern with expanding and collapsing nodes that contain nested items."));
...
void FluAllSamplesPage::makeDateAndTimeCards()
{
    ...
    auto calendarViewCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/CalendarView.png")), tr("CalendarView"), tr("A control that presents aa calendar for a user to choose a date from."));
    calendarViewCard->setKey("CalendarViewPage");
    m_cards.push_back(calendarViewCard);

    auto datePickerCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/DatePicker.png")), tr("DatePicker"), tr("A control that lets a user pick a date value."));
    datePickerCard->setKey("DatePickerPage");
    m_cards.push_back(datePickerCard);

    auto timePickerCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/TimePicker.png")), tr("TimePicker"), tr("A configurable control that lets a user pick a time value."));
...
void FluAllSamplesPage::makeLayoutCards()
{
    ...
    auto canvasCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/Canvas.png")), tr("Canvas"), tr("A layout panel that supports absolute positioning of child elements relative to the top left corner of the canvas."));
    canvasCard->setKey("CanvasPage");
    m_cards.push_back(canvasCard);

    auto expanderCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/Expander.png")), tr("Expander"), tr("A container with a header that can be expanded to show a body with more content."));
    expanderCard->setKey("ExpanderPage");
    m_cards.push_back(expanderCard);

    auto gridCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/Grid.png")), tr("Grid"), tr("A flexible, primitive control for data-driven layouts."));
    gridCard->setKey("GridPage");
    m_cards.push_back(gridCard);

    auto itemsRepeaterCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ItemsRepeater.png")), tr("ItemsRepeater"), tr("A flexible, primitive control for data-driven layouts."));
    itemsRepeaterCard->setKey("ItemRepeater");
    m_cards.push_back(itemsRepeaterCard);

    auto radioButtonsCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/RadioButtons.png")), tr("RadioButtons"), tr("A control that displays a group of mutually exclusive options with keyboarding and accessibility support."));
    radioButtonsCard->setKey("RadioButtonsPage");
    m_cards.push_back(radioButtonsCard);

    auto relativePanelCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/RelativePanel.png")), tr("RelativePanel"), tr("A panel that users relationships between elements to define layout."));
    relativePanelCard->setKey("RelativePanelPage");
    m_cards.push_back(relativePanelCard);

    auto splitViewCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/SplitView.png")), tr("SplitView"), tr("A container that has 2 content areas, with multiple display options for the pane."));
    splitViewCard->setKey("SplitViewPage");
    m_cards.push_back(splitViewCard);

    auto stackPanelCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/StackPanel.png")), tr("StackPanel"), tr("A layout panel that arranges child elements into a single line that can be oriented horizontally or vertically."));
    ...
    auto viewboxCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/Viewbox.png")), tr("Viewbox"), tr("A container control that scales its content to a specified size."));
...
void FluAllSamplesPage::makeMediaCards()
{
    ...
    auto captureElementAndCameraPreviewCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/CaptureElement.png")), tr("Capture Element / Camera Perview"), tr("A sample for doing a camera preview."));
    ...
    auto imageCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/Image.png")), tr("Image"), tr("A control to display image content."));
    imageCard->setKey("ImagePage");
    m_cards.push_back(imageCard);

    auto mapControlCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/MapControl.png")), tr("MapControl"), tr("Display a symbolic map of the Earth."));
    mapControlCard->setKey("MapControlPage");
    m_cards.push_back(mapControlCard);

    auto mediaPlayerElementCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/MediaPlayerElement.png")), tr("MediaPlayerElement"), tr("A control to display video and image content."));
    mediaPlayerElementCard->setKey("MediaPlayerElementPage");
    m_cards.push_back(mediaPlayerElementCard);

    auto personPictureCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/PersonPicture.png")), tr("PersonPicture"), tr("Displays the picture of a person/contact."));
    personPictureCard->setKey("PersonPicturePage");
    m_cards.push_back(personPictureCard);

    auto soundCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/Sound.png")), tr("Sound"), tr("A code-behind only API that enables 2D and 3D UI sounds on all XAML controls."));
    soundCard->setKey("SoundPage");
    m_cards.push_back(soundCard);

    auto webView2Card = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/WebView.png")), tr("WebView2"), tr("A Microsoft Edge(Chrominum) based control that hosts HTML content in an app."));
...
void FluAllSamplesPage::makeScrollingCards()
{
    auto annotatedScrollBarCard =
        new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/AnnotatedScrollBar.png")), tr("AnnotatedScrollBar"), tr("A control that extends a regular vertical scrollbar's functionality for an easy navigation through large collections."));
    annotatedScrollBarCard->setKey("AnnotatedScrollBarPage");
    m_cards.push_back(annotatedScrollBarCard);

    auto pipsPagerCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/PipsPager.png")), tr("PipsPager"), tr("A control to let the user navigate through a paginated collection when the page numbers do not need to be visually known."));
    pipsPagerCard->setKey("PipsPagerPage");
    m_cards.push_back(pipsPagerCard);

    auto scrollViewCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ScrollView.png")), tr("ScrollView"), tr("A container control that lets the user pan and zoom its content."));
    scrollViewCard->setKey("ScrollViewPage");
    m_cards.push_back(scrollViewCard);

    auto scrollViewerCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ScrollViewer.png")), tr("ScrollViewer"), tr("A container control that lets the user pan and zoom its content."));
...
void FluAllSamplesPage::makeStatusInfoCards()
{
    ...
    auto progressRingCard = new FluHCard(FluIconUtils::getPixmap(("../res/ControlImages/ProgressRing.png")), tr("ProgressRing"), tr("Shows the apps progress on a task, or that the app is performing ongoing work that does block user interaction."));
