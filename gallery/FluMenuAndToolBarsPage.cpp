...
FluMenuAndToolBarsPage::FluMenuAndToolBarsPage(QWidget* parent /*= nullptr*/) : FluATitlePage(parent)
{
    ...
    addHCard(FluIconUtils::getPixmap(("../res/ControlImages/AppBarToggleButton.png")), tr("AppBarToggleButton"), tr("A button that can be on, off, or indeterminate like a CheckBox, and is styled for use in an app bar or other specialized UI."), "AppBarToggleButtonPage");
    ...
    addHCard(FluIconUtils::getPixmap(("../res/ControlImages/StandardUICommand.png")), tr("StandardUICommand"), tr("A StandardUICommand is a built-in 'XamlUICommand' which represents a commonly used command, e.g.'save'."), tr("StandardUICommandPage"));
