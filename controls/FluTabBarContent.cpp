...
FluTabBarContent::FluTabBarContent(QWidget* parent /*= nullptr*/) : QScrollArea(parent)
{
    ...
    m_mainWidget->setFixedHeight(30);
    setFixedHeight(30);
...
{
    // Consumed by FluTabView's junction overlay; mirrors the selected
    // background color declared in the FluTabBarItem.qss themes.
    if (FluThemeUtils::isLightTheme())
        return QColor(255, 255, 255);
    if (FluThemeUtils::isDarkTheme())
        return QColor(50, 50, 50);
    return QColor(54, 58, 66);  // AtomOneDark
}
